...
2. 通过 decorator 把信息标注在实际代码上，并由此生成 swagger 文件：依赖了具体实现的框架
...
这里根据之前定义的元数据，生成 json schema（生成脚本 generate-restful-api-handler.ts），然后用 json schema 来验证输入，json schema 作为一个验证标准，使用时不依赖具体的验证库，更具通用性。
...
5. 尽量只生成类型代码，不生成实现代码，以免代码生成脚本过于复杂、难以维护、耗时太长，也避免和具体前端请求库（fetch、axios）、具体后端框架（express、koa、nestjs）的耦合，这样更具通用性：✅ 生成脚本只有 53 + 129 行代码，所有 schema 的生成只需要 4.5 秒左右，前后端的类型都是足够抽象的，与使用的框架、数据库等都无关，只专注解决 API 相关的重复性劳动
